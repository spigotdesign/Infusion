<article id="post-<?php the_ID(); ?>" class="<?php hybrid_entry_class(); ?>">
	...
	<?php } else { ?>
		...
		<div class="entry-summary">
			...
			<?php if ( has_excerpt() ) {
				the_excerpt();
				wp_link_pages( array( 'before' => '<p class="page-links">' . '<span class="before">' . __( 'Pages:', 'infusion' ) . '</span>', 'after' => '</p>' ) );
			} ?>

			<?php $count = post_format_tools_get_image_attachment_count(); ?>
			<p class="image-count"><?php printf( _n( 'This gallery contains %d image.', 'This gallery contains %d images.', $count, 'infusion' ), $count ); ?></p>

		</div><!-- .entry-summary -->

		<footer class="entry-footer">
			<?php echo apply_atomic_shortcode( 'entry_meta', '<div class="entry-meta">' . __( '[post-format-link] published on [entry-published] [entry-comments-link before="| "] [entry-edit-link before="| "]', 'infusion' ) . '</div>' ); ?>
		</footer><!-- .entry-footer -->
